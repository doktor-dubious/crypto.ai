...
def _batch_loss(nn_module, ctx_batch, tgt_batch, context_length, horizon, device):
    """MOIRAI-2 per-batch MSE loss (native loss() if available, else forward+MSE)."""
...
def _eval_avg_loss(nn_module, loader, context_length, horizon, device) -> float:
    """Average loss over a loader with no gradient updates (held-out validation)."""
    nn_module.eval()
    total = 0.0
    n = 0
    with torch.no_grad():
        for ctx_batch, tgt_batch in loader:
            ...
            loss = _batch_loss(nn_module, ctx_batch, tgt_batch, context_length, horizon, device)
            total += loss.item()
            n += 1
...
def _train_outlet(
    module, series: np.ndarray, context_length, horizon, epochs, lr, batch_size,
    patience: int = 0, early_stopping_method: str = "training", validation_split: float = 0.0,
):
    import copy
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loader is not None:
            val_avg = _eval_avg_loss(nn_module, val_loader, context_length, horizon, device)
            logger.info(
        ...
        # Keep the best-scoring epoch's weights so the saved checkpoint is the
        # best one, not whichever epoch we happen to stop on.
        ...
        # Patience applies to the training-loss method only; the validation
        # method runs the full epoch budget and keeps the best-validation epoch.
...
async def run_finetune(
    ...
    # Accepted for call-signature parity with the TimesFM runner. MOIRAI-2's
    # training loss isn't on the same scale as TimesFM's normalized MSE, so the
    # TimesFM-tuned pathological threshold isn't applied here.
    ...
    if not has_local and sync_target:
        # rsync can take minutes — keep it off the event loop (matches the
        # TimesFM runner) so progress/heartbeat persistence isn't starved.
        await asyncio.get_event_loop().run_in_executor(
            None, _pull_checkpoint, output_dir, sync_target
        )
        has_local = os.path.isdir(output_dir) and any(
            f.endswith((".safetensors", ".bin")) for f in os.listdir(output_dir)
        )
    ...
    for outlet_id, values in outlet_series.items():
        ...
        logger.info("  Training on series %s (%d points)", outlet_id, len(series))
        trained = await asyncio.get_event_loop().run_in_executor(
            ...
            early_stopping_patience, early_stopping_method, validation_split,
        )
        if trained:
    ...
    logger.info("MOIRAI-2 fine-tuning done: %d/%d series trained", processed, total)
    return False
