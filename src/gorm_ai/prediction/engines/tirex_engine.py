...
class TiRexEngine(PredictionEngine):
    ...
    def estimate_memory(self, *, task_type: str = "prediction", num_outlets: int = 1,
        ...
        # TiRex: 35M params xLSTM-based, outputs quantiles directly
        bytes_per_param = 4  # TiRex uses float32
        model_mb = 35e6 * bytes_per_param / (1024 * 1024)
        ...
        inference_mb = context_mb + output_mb + ridge_mb + 100
    ...
    async def predict_batch(
        ...
        if self._model is None:
            if not self.allow_fallback:
                raise RuntimeError("TiRex model failed to load and engine fallback is disabled")
