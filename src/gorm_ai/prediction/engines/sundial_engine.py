...
class SundialEngine(PredictionEngine):
    ...
    def estimate_memory(self, *, task_type: str = "prediction", num_outlets: int = 1,
        ...
        # Sundial: 128M params, decoder-only with flow-matching
        bytes_per_param = 2 if precision in ("bfloat16", "float16") else 4
        model_mb = 128e6 * bytes_per_param / (1024 * 1024)
        ...
        # Flow-matching: num_samples output paths
        samples_mb = effective_batch * self._num_samples * horizon * 4 / (1024 * 1024)
        ridge_mb = effective_batch * context_length * max(num_covariates, 1) * 8 / (1024 * 1024)
        inference_mb = context_mb + samples_mb + ridge_mb + 200
        ...
        return MemoryEstimate(
            ...
            breakdown={"model": round(model_mb, 1), "context": round(context_mb, 1),
                       "samples": round(samples_mb, 1), "ridge": round(ridge_mb, 1),
    ...
    async def predict_batch(
        ...
        if self._model is None:
            if not self.allow_fallback:
                raise RuntimeError("Sundial model failed to load and engine fallback is disabled")
