...
type SortField = "date" | "return_pct" | "sold_out_pct" | "delivered" | "sold" | "returned" | "outlet_count" | "sold_out_count" | "starred"
...
const chartConfig: ChartConfig = {
  return_pct:   { label: "Return %",   color: "hsl(38 92% 50%)" },
  sold_out_pct: { label: "Sold Out %", color: "hsl(0 72% 51%)" },
}
...
const SE_PREFIX = "gorm:statsSoldOut:"
function loadSeV<T>(cid: string, k: string, fb: T): T { if (typeof window === "undefined") return fb; try { const r = localStorage.getItem(`${SE_PREFIX}${cid}:${k}`); return r ? JSON.parse(r) : fb } catch { return fb } }
function saveSeV(cid: string, k: string, v: unknown) { if (typeof window !== "undefined") localStorage.setItem(`${SE_PREFIX}${cid}:${k}`, JSON.stringify(v)) }

export default function StatisticsEfficiencyPage() {
  const t = useTranslations("statisticsEfficiency")
  const { activeCustomer } = useCustomer()
  const customerId = activeCustomer?.id
  const cid = customerId ?? ""

  // ── Filters (persisted) ──────────────────────────────────────────────────
  const [selectedGroupId, setSelectedGroupId] = useState<string | null>(() => loadSeV<string | null>(cid, "groupId", null))
  const [weeks, setWeeks] = useState(() => loadSeV<number>(cid, "weeks", 8))
  const [dateRange, setDateRange] = useState<DateRange | undefined>(undefined)
  const [weekday, setWeekday] = useState<string>(() => loadSeV<string>(cid, "weekday", "all"))
  ...
  // ── Persist filters ────────────────────────────────────────────────────────
  useEffect(() => { if (cid) saveSeV(cid, "groupId", selectedGroupId) }, [cid, selectedGroupId])
  useEffect(() => { if (cid) saveSeV(cid, "weeks", weeks) }, [cid, weeks])
  useEffect(() => { if (cid) saveSeV(cid, "weekday", weekday) }, [cid, weekday])
  const prevCidRef = useRef(cid)
  useEffect(() => { if (prevCidRef.current && cid && prevCidRef.current !== cid) { setSelectedGroupId(loadSeV<string | null>(cid, "groupId", null)); setWeeks(loadSeV(cid, "weeks", 8)); setWeekday(loadSeV(cid, "weekday", "all")) }; prevCidRef.current = cid }, [cid])
  ...
  const chartData = useMemo(() => {
    ...
    return efficiencyData.data
      ...
      .map((s) => ({
        ...
        sold_out_pct: s.sold_out_pct,
        delivered: s.delivered,
        sold: s.sold,
        returned: s.returned,
        outlet_count: s.outlet_count,
        sold_out_count: s.sold_out_count,
      }))
  ...
  const exportColumns: ExportColumn[] = useMemo(() => [
    ...
    { header: t("returnPct"), accessor: (r: any) => r.return_pct != null ? r.return_pct.toFixed(1) : "" },
    { header: t("soldOutPct"), accessor: (r: any) => r.sold_out_pct != null ? r.sold_out_pct.toFixed(1) : "" },
    { header: t("delivered"), accessor: (r: any) => r.delivered != null ? String(r.delivered) : "" },
    ...
    { header: t("soldOutCount"), accessor: (r: any) => String(r.sold_out_count) },
  ], [t])
        ...
        <ChartContainer config={chartConfig} className={cn("h-80 w-full aspect-auto transition-opacity duration-200", datePickerOpen && "opacity-10")}>
          <LineChart data={chartData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
            ...
            <ChartLegend
              content={() => (
                <div className="flex items-center justify-center gap-4 pt-2">
                  {(["return_pct", "sold_out_pct"] as const).map((key) => {
            ...
            <Line
              type="monotone"
              dataKey="return_pct"
              stroke={chartConfig.return_pct.color}
              strokeWidth={2}
              dot={{ r: 2 }}
              activeDot={{ r: 4 }}
              connectNulls
              hide={hiddenSeries.has("return_pct")}
            />
            <Line
              type="monotone"
              dataKey="sold_out_pct"
              stroke={chartConfig.sold_out_pct.color}
              strokeWidth={2}
              dot={{ r: 2 }}
              activeDot={{ r: 4 }}
              connectNulls
              hide={hiddenSeries.has("sold_out_pct")}
            />
          </LineChart>
        </ChartContainer>
      )}
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <ExportMenu
              ...
              filename={`efficiency-${effectiveGroup?.name ?? "all"}`}
            />
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="cursor-pointer select-none text-xs text-right" onClick={() => handleSort("return_pct")}>
                    <span className="flex items-center justify-end">{t("returnPct")}<SortIcon field="return_pct" /></span>
                  </TableHead>
                  <TableHead className="cursor-pointer select-none text-xs text-right" onClick={() => handleSort("sold_out_pct")}>
                    <span className="flex items-center justify-end">{t("soldOutPct")}<SortIcon field="sold_out_pct" /></span>
                  </TableHead>
                  <TableHead className="cursor-pointer select-none text-xs text-right" onClick={() => handleSort("delivered")}>
                    <span className="flex items-center justify-end">{t("delivered")}<SortIcon field="delivered" /></span>
                  </TableHead>
                  <TableHead className="cursor-pointer select-none text-xs text-right" onClick={() => handleSort("sold")}>
                    <span className="flex items-center justify-end">{t("sold")}<SortIcon field="sold" /></span>
                  </TableHead>
                  <TableHead className="cursor-pointer select-none text-xs text-right" onClick={() => handleSort("returned")}>
                    <span className="flex items-center justify-end">{t("returned")}<SortIcon field="returned" /></span>
                  </TableHead>
                  <TableHead className="cursor-pointer select-none text-xs text-right" onClick={() => handleSort("sold_out_count")}>
                    <span className="flex items-center justify-end">{t("soldOutCount")}<SortIcon field="sold_out_count" /></span>
                  </TableHead>
                  <TableHead className="w-10 px-3" />
                </TableRow>
              </TableHeader>
              <TableBody>
                {pagedRows.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={9} className="text-center text-xs text-[var(--muted-foreground)] py-8">
                      {t("noResults")}
                    </TableCell>
                  </TableRow>
                ) : (
                  pagedRows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="text-xs text-right tabular-nums">{fmtPct(row.return_pct)}</TableCell>
                        <TableCell className="text-xs text-right tabular-nums">{fmtPct(row.sold_out_pct)}</TableCell>
                        <TableCell className="text-xs text-right tabular-nums">
                          {row.delivered != null ? row.delivered.toLocaleString() : "\u2014"}
                        </TableCell>
                        <TableCell className="text-xs text-right tabular-nums">{row.sold.toLocaleString()}</TableCell>
                        <TableCell className="text-xs text-right tabular-nums">
                          {row.returned != null ? row.returned.toLocaleString() : "\u2014"}
                        </TableCell>
                        <TableCell className="text-xs text-right tabular-nums">
                          {row.sold_out_count}/{row.outlet_count}
                        </TableCell>
                        <TableCell className="px-3">
                          <button
