...
const SCISSORS_VARIANTS: Variants = {
  normal: { rotate: 0, scale: 1 },
  animate: {
    rotate: [0, -8, 8, -4, 0],
    scale: [1, 0.95, 1.05, 1],
...
export function AnimatedScissors({ className, controls: externalControls }: { className?: string; controls?: ReturnType<typeof useAnimation> }) {
  const internalControls = useAnimation()
  const controls = externalControls ?? internalControls
  const mouseHandlers = externalControls ? {} : {
    onMouseEnter: () => controls.start("animate"),
