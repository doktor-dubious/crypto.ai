...
interface OptimalSettingsModalProps {
  open: boolean
...
const SETTINGS: Setting[] = [
  ...
  { type: "toggle", key: "optimize_covariate_handling", labelKey: "optimizeCovariateHandling", optionCount: 3 },
...
export function OptimalSettingsModal({ open, onOpenChange }: OptimalSettingsModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t("optimizeModalTitle")}</DialogTitle>
          <DialogDescription>{t("optimizeModalDescription")}</DialogDescription>
        </DialogHeader>

        <div className="space-y-1 py-2">
          <label className="text-xs font-medium text-muted-foreground">{t("optimizeName" as Parameters<typeof t>[0])}</label>
          <Input
            value={name}
        ...
        <div className="space-y-2 py-2">
          {SETTINGS.map((setting) => (
            <div key={setting.key}>
              ...
              {/* Range inputs shown when enabled */}
