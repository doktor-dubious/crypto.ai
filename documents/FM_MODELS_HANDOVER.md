...
- **Load + inference (`predict_df`):**
  ```python
  from chronos import Chronos2Pipeline
  pipe = Chronos2Pipeline.from_pretrained("amazon/chronos-2", device_map="cuda")  # or "cpu"
  # df: long format with id/timestamp/target columns (+ optional past/future covariate columns)
  out = pipe.predict_df(df, prediction_length=horizon, quantile_levels=[0.1,...,0.9])
  ```
  Verify the exact `predict_df` parameter names (`prediction_length`, `quantile_levels`, covariate columns) against the current `chronos-forecasting` README — the 2.x API was still settling.
